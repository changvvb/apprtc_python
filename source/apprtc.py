...
def make_pc_config(ice_transports):
  config = {
  'iceServers': [
#	{'url':'stun:60.205.93.75'},
#	{'url':'stun:stun.voxgratia.org'},
#	{'url':'stun:stun.voipbuster.com'},
#	{'url':'turn:60.205.93.75'}
],
...
def get_room_parameters(request, room_id, client_id, is_initiator):
  ...
  if len(ice_server_base_url) > 0:
	...
	(ice_server_base_url,username ,constants.CEOD_KEY)
...
#ice_server_url = constants.TURN_URL_TEMPLATE % \
 #    (ice_server_base_url, username, constants.CEOD_KEY)
...
def save_message_from_client(host, room_id, client_id, message):
  ...
  key = get_memcache_key_for_room(host, room_id)
  memcache_client = memcache.Client()
  retries = 0  # Compare and set retry loop.
  while True:
    room = memcache_client.gets(key)
    if room is None:
...
class DeleteRoomPage(webapp2.RequestHandler):
    def post(self, room_id):
        deleteRoom(self.request.host_url, room_id)
        #if result['error'] is None:
        #    logging.info('Room ' + room_id + ' has state ' + result['room_state'])
    def get(self, room_id):
        self.post(self,room_id)

def deleteRoom(host,room_id):
    key = get_memcache_key_for_room(host, room_id)
    memcache_client = memcache.Client()
    room = memcache_client.gets(key)
    if room is not None:
        logging.info('room is not null                         ---')
        room.clear_clients()
        if memcache_client.cas(key, room, constants.ROOM_MEMCACHE_EXPIRATION_SEC):
            logging.info('Removed from room room_id = %s' \
                         %(room_id))
        room.kill()
    else: 
        logging.info("room is null.....")
        #return {'error': constants.RESPONSE_UNKNOWN_ROOM, 'room_state': None}
#    else:
#       room.kill(room)
#    logging.info('deleteRoom:' + room_id)
...
app = webapp2.WSGIApplication([
    ...
    ('/deleteRoom/([a-zA-Z0-9-_]+)',DeleteRoomPage),
], debug=True)
